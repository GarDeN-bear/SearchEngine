...
SqlDatabase::SqlDatabase(const std::string host, const std::string port,
  ...
  try {
    c_ = std::make_unique<pqxx::connection>(
        "host=" + host_ + " " + "port=" + port_ + " " + "dbname=" + dbname_ +
...
void SqlDatabase::addClient(std::string table, std::string first_name,
  ...
  std::string insert =
      ...
      tx.esc(first_name) + "', '" + tx.esc(last_name) + "', '" + tx.esc(email) +
      "', '" + tx.esc(phone_number) + "')";
  tx.exec(insert);
  tx.commit();
